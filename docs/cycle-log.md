...
- タスク選定: `### 4`・`### 12`は引き続き素材待ちでスキップ。`### 13`書き直し
  タスクの続きとして、対象リスト先頭の3件(学院祭、優勝者に贈られるもの/
  記憶を持つ本、答えたくない問い/時間経路干渉、再現実験室の外側)を
  まとめて選定。予告通り、いずれもflavor未着手の単発root行だった。
- 実装: 3ページとも本文は既に「誰の視点でもない客観的な資料の記述」という
  許容形式で書かれ、定型フックも無かったため各カードは変更なし。
  champions-prizes.htmlのみ、2つの行事の副賞を比較する構成のため導入文に
  「学院祭の運営を手伝う職員」という書き手を明記した。memory-books.html・
  time-path-interference.htmlは単一テーマのフィールドノートとして既に
  自然な体裁だったため導入文も変更なし(すべてのページに無理に書き手を
  付与する必要はないと判断)。`docs/roadmap-done.md`「### 13」に詳細を
  追記し、`docs/ROADMAP.md`の対象リストを7件に更新。
- レビュー: OK(local-review観点で目視確認、指摘なし)。
- lint: ✓ / lint:css: ✓ / test: ✓ / build: ✓
- 次回予定: `### 13`書き直しの続き。対象リスト先頭の`eldcloth-fabric.html`
  から残り7件(garigne-unit/summoning-theory/starfall-fortune/
  machinery-department/apprentice-guides/east-lot-history)まで、内容確認
  しつつ進める。旧・定型フック使用の疑いは無い見込み。
- blocked / partial: なし
- asset-pending: なし
...
- ブランチ: 引き続き`evolve/cycle-41`(未マージ)。
- タスク選定: `### 4`・`### 12`は引き続き素材待ちでスキップ。`### 13`書き直し
  タスクの続きとして、対象リスト先頭のP62→P63チェーン(隠し材料、三つの
  脇役/魔力ナッツ仕入れ記録、地図に無い仕入れ先)をまとめて選定。
- 実装: P62(カード3)で旧・定型フックの文言が使われていた。「学食の仕入れ
  担当」という語り手を新設し、産地を聞かれて言葉を濁す担当者自身のやり取り
  として「魔力ナッツ仕入れ記録」を埋め込み直した。P63側は既に許容形式・
  定型フック無しで適合済みだったため本文は変更なし、導入文のみ同じ担当者が
  記録を確かめに行ったという体に軽く繋げた。**これで旧・定型フック使用が
  疑われていた8件(P44・P47・P49・P51・P53・P56・P59・P62)すべての対応が
  完了した。** `docs/roadmap-done.md`「### 13」に詳細を追記し、
  `docs/ROADMAP.md`の対象リストを10件に更新。
- レビュー: OK(local-review観点で目視確認。検索キーワード「魔力ナッツ
  仕入れ記録」が正しく残っていることを確認済み)。
- lint: ✓ / lint:css: ✓ / test: ✓ / build: ✓
- 次回予定: `### 13`書き直しの続き。残り10件はいずれも単発root行(flavorを
  持たない、または単語1つのみのroot→flavor 1段構成)で、これまでの調査上
  旧・定型フック使用の疑いは無い(実装記録に「単語1つのみだが、2〜3カード
  構成の方針に沿い」という記載が多く、既に許容形式で書かれている見込み)。
  対象リスト先頭の`champions-prizes.html`(P66、flavorはP67〜P69未着手)
  から順に、内容確認のみ軽く行いつつ進める。
- blocked / partial: なし
- asset-pending: なし
...
- ブランチ: 引き続き`evolve/cycle-41`(未マージ)。
- タスク選定: `### 4`・`### 12`は引き続き素材待ちでスキップ。`### 13`書き直し
  タスクの続きとして、対象リスト先頭のP59→P60→P61の3段チェーン
  (決闘演武場、力を支える三つのしるし/法典余白の書き込み、紋様の原型らしき
  一文/来場者記帳簿、同じ癖の「る」)をまとめて選定。
- 実装: P59(カード3)・P60(カード2)の2箇所で旧・定型フックの文言が使われて
  いた。「演武場の指導教官」という語り手を新設し、P59は教官自身の発言として
  「法典余白の書き込み」を、P60は同じ教官が「今度めくってみようと思う」
  という体で「来場者記帳簿」を、それぞれ自然な文脈に埋め込み直した。P61は
  既に許容形式・定型フック無しで、名前だけでは何者かまで辿れないという
  新たな行き止まりを残す形で意図的に完全解決を避けていたため変更なし。
  `docs/roadmap-done.md`「### 13」に詳細を追記し、`docs/ROADMAP.md`の
  対象リストを12件に更新。
- レビュー: OK(local-review観点で目視確認。検索キーワード「法典余白の
  書き込み」「来場者記帳簿」が正しく残っていることを確認済み)。
- lint: ✓ / lint:css: ✓ / test: ✓ / build: ✓
- 次回予定: `### 13`書き直しの続き。対象リスト先頭の`hidden-ingredients.html`
  (P62)から着手。P62は最初の実装時に検索可能語句「魔力ナッツ仕入れ記録」を
  手がかりとして追記済みだが、当時の言い回しが定型フックのパターンに
  当たっていないか本文を確認する必要がある。旧・定型フック使用が疑われた
  8件(P44・P47・P49・P51・P53・P56・P59・P62)のうち、これが最後の1件。
- blocked / partial: なし
- asset-pending: なし
...
- ブランチ: 引き続き`evolve/cycle-41`(未マージ)。
- タスク選定: `### 4`・`### 12`は引き続き素材待ちでスキップ。`### 13`書き直し
  タスクの続きとして、対象リスト先頭のP56→P57→P58の3段チェーン
  (大結界召喚陣、力を集める四つの石/連動調整記録、揃わない出力の理由/
  石材鑑定書、石は無実だったという結論)をまとめて選定。
- 実装: P56(カード2)・P57(カード2)の2箇所で旧・定型フックの文言が使われて
  いた。「儀式のたびに調整卓で出力を合わせる広場の担当者」という語り手を
  新設し、P56は担当者自身の発言として「連動調整記録」を、P57は同じ担当者が
  「今度読んでみるつもり」という体で「石材鑑定書」を、それぞれ自然な文脈に
  埋め込み直した。P58は元々P56〜P58で唯一遡及修正を要さなかったページ
  (外部鑑定士による調査という体裁が最初から定型フックを使っていなかった)
  であり、今回も変更なし。`docs/roadmap-done.md`「### 13」に詳細を追記し、
  `docs/ROADMAP.md`の対象リストを15件に更新。
- レビュー: OK(local-review観点で目視確認。検索キーワード「連動調整記録」
  「石材鑑定書」が正しく残っていることを確認済み)。
- lint: ✓ / lint:css: ✓ / test: ✓ / build: ✓
- 次回予定: `### 13`書き直しの続き。対象リスト先頭の`dueling-heritage.html`
  (P59)とそのflavor`margin-note.html`(P60)・`visitor-log-match.html`
  (P61)の3段チェーンから着手。同様に旧・定型フック使用の疑いがある。
- blocked / partial: なし
- asset-pending: なし

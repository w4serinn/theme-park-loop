...
### グループA: 「刻」三部作(命名パターンの呼応)
- 大魔法陣「刻の輪」(summoning-plaza.html)
- 管理台帳「刻の書」(clock-tower.html)
- 大鐘「刻の声」(clock-tower.html)
→ 3項目ちょうど。広場と時計塔という2エリアから接続できる網状構造も組める
(なぜ離れた2エリアの装置が同じ「刻」の字を冠するのか、という謎を核にできる)。

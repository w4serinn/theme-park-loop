...
- 書き直し完了: glossary/final-entry.html (2026-07-30、既にオーレン本人の手記の
  引用形式だったため変更なし)
- 書き直し完了: glossary/shooting-star.html (2026-07-30、根拠のない推測が
  誰の視点でもない地の文で書かれていたため、手記を発見した資料整理係の
  余白書き込みとして書き直し、final-entry/gear-cipherとは別の語り手を追加)
- 書き直し完了: glossary/erased-champion.html (2026-07-30、既に「誰の視点でも
  ない客観的な資料の記述」という許容形式で書かれ定型フックも無かったため
  変更なし)
- 書き直し完了: glossary/yorishiro-echo.html (2026-07-30、ノスティオン自身の
  真名にまつわるP91チェーンの最終ページであり、ノスティオン自身の声で
  書かれているのは意図的な例外として妥当と判断、変更なし)
- 書き直し完了: glossary/koku-trio.html (2026-07-31、6カード本文は既に
  「誰の視点でもない客観的な資料の記述」という許容形式で定型フックも無く
  適合済みだったため各カードは変更なし。導入文のみ、この比較資料を
  まとめた「学院史料編纂室の職員」という書き手を明記し、無記名の資料から
  一次資料への体裁に寄せた)
- 書き直し完了: glossary/beyond-the-map.html (2026-07-31、4カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  羅針堂で地図更新を担当する職員の覚え書きという書き手を明記した)
- 書き直し完了: glossary/arnold-relics.html (2026-07-31、6カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  購買部の品揃え担当者という書き手を明記した)
- 書き直し完了: glossary/spirits-of-arnold.html (2026-07-31、6カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  学院新聞部の取材記事という体裁を明記した)
- 書き直し完了: glossary/circle-ledgers.html (2026-07-31、4カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  後に陣紋師となる研究生の覚え書きという書き手を明記した(P13
  circle-warden.htmlの陣紋師チェーンとも自然に接続))
- 書き直し完了: glossary/merit-records.html (2026-07-31、4カード本文は既に
  許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  学院事務局の記録管理担当者という書き手を明記した)
- 書き直し完了: glossary/second-headmaster.html (2026-07-31、4カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  復刻版ペンを扱う写本堂の店主という書き手を明記した)
- 書き直し完了: glossary/hidden-corners.html (2026-07-31、8カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  六つの一角を巡り歩いた学生の課外活動記録という体裁を明記した)
- 書き直し完了: glossary/moon-grass.html (2026-07-31、3カード本文は既に
  許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  学食・購買部の仕入れ担当職員という書き手を明記した)
- 書き直し完了: glossary/scribe-lineage.html (2026-07-31、4カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  大図書館の司書という書き手を明記した)
- 書き直し完了: glossary/aptitude-certificate.html (2026-07-31、3カード本文は
  既に許容形式で定型フックも無く適合済みだったため変更なし。導入文のみ、
  魔導具店の店員という書き手を明記した)
- 書き直し完了: glossary/circle-warden.html・warden-registry.html・
  unnamed-warden.html (2026-07-31、P13→P14→P15の陣紋師チェーン3ページ。
  各本文は既に許容形式で定型フックも無く適合済みだったため変更なし。3ページ
  とも導入文のみ「広場管理事務所の事務員」という同一の書き手に統一し、
  チェーンとしての一貫性を持たせた)
- 書き直し完了: glossary/arnold-namesake.html・amber-heart-blueprint.html
  (2026-07-31、P44→P45チェーン。この2ページは、旧・定型フック
  (「〜には〜が残っているというから、そこに手がかりがあるのかもしれない」の
  類型)が実際に使われていた最初のケース。検索キーワード「設計図台帳」を
  カード末尾の告知文から、蒸留器を保守する技師の発言・調査記録という自然な
  文脈に埋め込み直した。P45側は当初「今度確かめてみます」という未解決の
  ままの終わり方だったが、技師が実際に台帳を調べた結果(「原本、貸出中」と
  だけ記載があり貸出先・返却記録は無し)を書き加え、新たな謎(誰が借りた
  のか)を残す形に変更。ノスティオンの独り言もこの結末に合わせて書き直した)
- 書き直し完了: glossary/airship-symbols.html・anchor-feather-origin.html
  (2026-07-31、P47→P48チェーン。P47は旧・定型フックの文言(「もっとも、
  〜には〜が残っているというから、そこに手がかりがあるのかもしれない」)が
  一字一句そのまま使われていたケース。検索キーワード「初代ドック長の航海
  日誌」を、現在のドック長自身の発言として埋め込み直した。P48側も、当初
  「シンボルマークが選ばれた経緯にも触れられているのではないかと考え
  られている」という推測止まりの終わり方だったが、ドック長が実際に日誌を
  読んだ結果(意匠の由来は結局見つからなかったが、日誌最後の余白に碇と
  羽根の落書きだけ残っていた)を書き加え、P44→P45と同型の「調べたが
  部分的にしか解決しない」終わり方に変更。ノスティオンの独り言もこの結末に
  合わせて書き直した)
- 書き直し完了: glossary/fifth-headmaster.html・portrait-gallery.html
  (2026-07-31、P49→P50チェーン。P49は旧・定型フックの文言が一字一句
  そのまま使われていたケース。検索キーワード「歴代学長肖像画帳」を、
  天文台の職員への聞き取りという体裁に埋め込み直した(「まだ確かめた
  ことがない」と返す職員自身の言葉にすることで、次のページへの導線が
  自然な会話の一部になるようにした)。P50(portrait-gallery.html)は
  既に「誰の視点でもない客観的な資料の記述」という許容形式で書かれ、
  肖像画は描かれているのに略歴欄だけ空白という新たな謎を残す形で
  意図的に完全解決を避けており、定型フックも無かったため変更なし)
- 書き直し完了: glossary/festival-undertold.html・fortune-teller-selection.html
  (2026-07-31、P51→P52チェーン。P51は旧・定型フックの文言が一字一句
  そのまま使われていたケース。検索キーワード「占術師選定記録」を、現職の
  占術師本人への聞き取りという体裁に埋め込み直した。P52
  (fortune-teller-selection.html)は既に許容形式・定型フック無しで、
  17代全員が「大点灯」当日生まれという新たな符合を残す形で意図的に
  完全解決を避けていたため変更なし)
- 書き直し完了: glossary/campus-harvest.html・orchard-experiment-log.html・
  conductivity-instrument-log.html (2026-07-31、P53→P54→P55の3段チェーン。
  P53(カード3)・P54(カード2)の2箇所で旧・定型フックの文言が使われていた。
  「林檎園の世話を任されている研究生」という語り手を新設し、P53は
  研究生自身の発言として「林檎栽培実験記録」を、P54は同じ研究生が
  「次に調べる先を決めた」という体で「伝導率測定器台帳」を、それぞれ
  カード末尾の告知文から自然な文脈に埋め込み直した。3ページを通して
  同じ研究生が記録→台帳と辿っていく一貫した調査の流れにするため、
  P55の導入文にも軽く触れた。P55本文自体は既に許容形式・定型フック
  無しで、異界微弱反応検知用の測定器という新たな謎を残す形で意図的に
  完全解決を避けていたため変更なし)
- 書き直し完了: glossary/four-elements-seal.html・resonance-adjustment-log.html
  (2026-07-31、P56→P57→P58の3段チェーン。P56(カード2)・P57(カード2)の
  2箇所で旧・定型フックの文言が使われていた。「儀式のたびに調整卓で出力を
  合わせる広場の担当者」という語り手を新設し、P56は担当者自身の発言として
  「連動調整記録」を、P57は同じ担当者が「今度読んでみるつもり」という体で
  「石材鑑定書」を、それぞれ自然な文脈に埋め込み直した。P58
  (stone-inspection-report.html)は元々P56〜P58で唯一遡及修正を要さなかった
  ページ(外部鑑定士による調査という体裁が最初から定型フックを使っていな
  かった)であり、今回も変更なし)
- 書き直し完了: glossary/dueling-heritage.html・margin-note.html
  (2026-07-31、P59→P60→P61の3段チェーン。P59(カード3)・P60(カード2)の
  2箇所で旧・定型フックの文言が使われていた。「演武場の指導教官」という
  語り手を新設し、P59は教官自身の発言として「法典余白の書き込み」を、
  P60は同じ教官が「今度めくってみようと思う」という体で「来場者記帳簿」を、
  それぞれ自然な文脈に埋め込み直した。P61(visitor-log-match.html)は既に
  許容形式・定型フック無しで、名前だけでは何者かまで辿れないという新たな
  行き止まりを残す形で意図的に完全解決を避けていたため変更なし)
- 書き直し完了: glossary/hidden-ingredients.html・nut-supply-record.html
  (2026-07-31、P62→P63チェーン。P62(カード3)で旧・定型フックの文言が
  使われていた。「学食の仕入れ担当」という語り手を新設し、産地を聞かれて
  「言葉を濁す」担当者自身のやり取りとして検索キーワード「魔力ナッツ仕入れ
  記録」を埋め込み直した。P63側は既に許容形式・定型フック無しで適合済み
  だったため本文は変更なし、導入文のみ同じ担当者が記録を確かめに行った
  という体に軽く繋げた。これで旧・定型フック使用が疑われていた8件
  (P44・P47・P49・P51・P53・P56・P59・P62)すべての対応が完了した)
- 書き直し完了: glossary/champions-prizes.html・memory-books.html・
  time-path-interference.html (2026-07-31、いずれも本文は既に「誰の視点でも
  ない客観的な資料の記述」という許容形式で定型フックも無く適合済みだった
  ため各カードは変更なし。champions-prizes.htmlのみ、2つの行事の副賞を
  比較する構成のため導入文に「学院祭の運営を手伝う職員」という書き手を
  明記した。memory-books.html・time-path-interference.htmlは単一テーマの
  フィールドノートとして既に自然な体裁だったため導入文も変更なし)
- 書き直し完了: glossary/eldcloth-fabric.html・garigne-unit.html・
  summoning-theory.html・starfall-fortune.html・machinery-department.html・
  apprentice-guides.html・east-lot-history.html (2026-07-31、いずれも本文は
  既に「誰の視点でもない客観的な資料の記述」という許容形式で定型フックも
  無く適合済みのため変更なし。いずれもflavor側が未着手のroot単発行であり、
  ライブな検索キーワード連鎖が存在しないため、書き手付与も行わなかった)
- [x] **既存隠しページ54件のライティング方針改訂に伴う全面書き直し、
  全件完了(2026-07-31)。** 定型文(「もっとも、〜には〜が残っている
  というから、そこに手がかりがあるのかもしれない」)を実際に使用していた
  8件(P44・P47・P49・P51・P53・P56・P59・P62、いずれもroot→flavor
  チェーンの2ページ1組)は、共通の語り手(蒸留器を保守する技師、広場の
  調整卓担当者、演武場の指導教官等)を各チェーンごとに新設し、検索
  キーワードを告知文から自然な会話・証言の文脈に埋め込み直した。残りの
  大多数(P1〜P63のうち上記8件を除く全て)は、実装当初から「誰の視点でも
  ない客観的な資料の記述」という許容形式、または書籍系ページの淡々とした
  記載で書かれており、定型フックも使われていなかったため、本文の書き直しは
  不要だった(該当箇所には判断根拠のみ記録)。図鑑・カタログ系ページ
  (魔法生物図鑑・魔導88星座)については証言形式への変換自体が誤りだった
  ため、書籍としての地の文に戻した。
